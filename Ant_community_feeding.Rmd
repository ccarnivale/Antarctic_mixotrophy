...
Currently, missing some environmental data and 1 rep at station A surface.
Also, station H has some incongruency with the other reps but I think those are just
bad filtered samples and nothing we can do about it.
...
Now that we have some proper factors I will group and summarise everything to get
a proper plot and a raw boxplot for our data while removing the one bad sample with no data.

```{r pre-visualization manipulation}
...
```{r Bacterivory: read file and tidying}
...
```{r Attempting Multivariate analysis}
...
For some of the stations, there is 0 calculated feeding for both HNAN and MNAN due to the high
amount of background from T0s from the doubling of microspheres added and increase in amount filtered.
This creates a situation where a percent calculation ends up as = 0/0 or NaN. For initial analysis purposes I will change all NaNs into 0s. I will revisit those slides with negative values to make sure there aren't any typos or slides I still need to reread.
...
Now there are some issues with this first PCA analysis. ALL variables were used including all
calculated and some biased variables (looking at you usphere concentration). This next chunk 
is to narrow down and remove some covariates and irrelevant variables.

```{r Refined PCA analysis with "functional" groups and bacterivory}
...
Seems there's a difference in groupings between all, functional, and bacterivory.
In all, the 4 main groupings still shine through, however, theres station Q which is coupling with the
Southern stations and SOME overlap between Northern, Offshore, and Southern stations. Other still remains
separate from the other stations.
...
```{r Redo using new background false positive method}
...
```{r looking at activity based on carbon only - merge %bprod fig}
...
```{r table for JD to check calculation + %bprod}
...
```{r Filters for raw ranges and avgs of dif subgroups (lat/depth)}
...
```{r a new df for %read est of abundance}
